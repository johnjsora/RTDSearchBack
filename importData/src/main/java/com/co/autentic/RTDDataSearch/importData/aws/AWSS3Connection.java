...
public class AWSS3Connection {
    ...
    public s3FileInformation getFileBytesFromKeyLast(String bucket, String key) {

        try {
            s3FileInformation  response = new s3FileInformation();
            S3Object object = this.awsS3Client.getObject(new GetObjectRequest(bucket, key));
            InputStream objectData = object.getObjectContent();
            ObjectMetadata metadata = object.getObjectMetadata();
            Date lastModified = metadata.getLastModified();
            response.setFile(IOUtils.toByteArray(objectData));
            response.setLastModifiedDate(lastModified);
            objectData.close();
